...
# Define the GPs to process
GPs_TO_PROCESS = [
    'S100A1_ligand_receptor_GP',
    'CXCL2_combined_GP',
    'CXCL9_combined_GP',
    'CXCL6_combined_GP',
...
def load_and_prepare_data(adata_path, gp_name):
    """Load and prepare the initial dataset for a specific GP"""
    adata = sc.read_h5ad(adata_path)
    
    # Extract pathway genes for the specific GP
    gp = pd.DataFrame(adata.uns['nichecompass_gp_summary'])
    current_GP = gp[gp['gp_name'] == gp_name]
    
    if current_GP.empty:
        raise ValueError(f"GP {gp_name} not found in the dataset")
    
    source_genes = current_GP['gp_source_genes'].values[0]
    target_genes = current_GP['gp_target_genes'].values[0]
...
def process_single_gp(gp_name, output_dir, adata_path):
    """Process a single gene program"""
    print(f"\nProcessing {gp_name}...")
    try:
        # Load and prepare data for this GP
        adata, pathway_genes = load_and_prepare_data(adata_path, gp_name)
        
        # Process real data
        results, final_data_frame, ch_scores, db_scores, inertias = process_real_data(
            adata, pathway_genes, output_dir, gp_name
        )
        ...
        boot_df, randomized_boot_df = save_results(
            results, randomized_results, output_dir, gp_name
        )
        ...
        print(f"Error processing {gp_name}: {str(e)}")
        return False
...
def main():
    ...
    # Process each GP
    results_summary = {}
    for gp_name in GPs_TO_PROCESS:
        success = process_single_gp(gp_name, output_dir, adata_path)
        results_summary[gp_name] = "Success" if success else "Failed"
    
    # Print summary
    print("\nProcessing Summary:")
    for gp_name, status in results_summary.items():
        print(f"{gp_name}: {status}")
